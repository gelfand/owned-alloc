use crate::{OwnedAlloc, UninitAlloc};

// /// Pointer to memory allocation that might be either initialized or
    ...
    T: ?Sized,
{
    ...
    #[inline]
    pub const fn uninit_as_ok(self) -> Result<UninitAlloc<T>, OwnedAlloc<T>> {
        match self {
...
impl<T> const From<OwnedAlloc<T>> for MaybeUninitAlloc<T>
where
    T: ?Sized,
{
    #[inline]
...
impl<T> const From<UninitAlloc<T>> for MaybeUninitAlloc<T>
where
    T: ?Sized,
{
    #[inline]
...
impl<T> core::fmt::Debug for MaybeUninitAlloc<T>
where
    T: ?Sized,
{
    #[inline]
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        match self {
            MaybeUninitAlloc::Init(ptr) => write!(f, "Init({:?})", ptr),
            MaybeUninitAlloc::Uninit(ptr) => write!(f, "Uninit({:?})", ptr),
        }
    }
}
